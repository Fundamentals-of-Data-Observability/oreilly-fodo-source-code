...
monthly_assets = pd.concat([AppTech, Buzzfeed]).astype({"Symbol": "category"})
monthly_assets.to_csv("data/monthly_assets.csv", index=False)

AppTech_ds = DataSource("data/AppTech.csv", "csv")
...
Buzzfeed_ds = DataSource("data/Buzzfeed.csv", "csv")
print(json.dumps(Buzzfeed_ds))
Buzzfeed_sc = Schema(Schema.extract_fields_from_dataframe(AppTech), Buzzfeed_ds)
print(json.dumps(Buzzfeed_sc))
monthly_assets_ds = DataSource("data/monthly_assets.csv", "csv")
print(json.dumps(monthly_assets_ds))
monthly_assets_sc = Schema(Schema.extract_fields_from_dataframe(AppTech), monthly_assets_ds)
print(json.dumps(monthly_assets_sc))
lineage = OutputDataLineage(monthly_assets_sc,
                            OutputDataLineage.generate_direct_mapping(monthly_assets_sc, [AppTech_sc, Buzzfeed_sc]))
print(json.dumps(lineage))
lineage_exe = DataLineageExecution(lineage, app_exe)
print(json.dumps(lineage_exe))
AppTech_ms = DataMetrics(DataMetrics.extract_metrics_from_df(AppTech), AppTech_sc, lineage_exe)
print(json.dumps(AppTech_ms))
Buzzfeed_ms = DataMetrics(DataMetrics.extract_metrics_from_df(Buzzfeed), Buzzfeed_sc, lineage_exe)
print(json.dumps(Buzzfeed_ms))
monthly_assets_ms = DataMetrics(DataMetrics.extract_metrics_from_df(monthly_assets), monthly_assets_sc, lineage_exe)
print(json.dumps(monthly_assets_ms))
